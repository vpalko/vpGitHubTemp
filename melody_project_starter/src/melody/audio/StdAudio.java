...
public final class StdAudio {
	...
	public static class AudioEvent {
		...
		public String toString() {
            ...
            stringBuilder.append("AudioEvent{Type=");
            stringBuilder.append(type);
            stringBuilder.append((note == null ? "" : (String.format(", note=%s", note))));
            stringBuilder.append((duration == 0.0 ? "" : (String.format(", duration=%f", duration))));
	...
	public static void loop(String filename) {
            ...
            if (url == null)
                throw new RuntimeException(String.format("audio %s not found", filename));
            AudioClip clip = Applet.newAudioClip(url); //Outdated
            clip.loop();
            /*AudioCue clip;
            try {
                clip = AudioCue.makeStereoCue(url, 4);
                clip.open();  // see API for parameters to override "sound thread" configuration defaults
                clip.play(1, 0, 1, 1);
                clip.close();
            } catch (Exception e) {
    ...
    public static void save(String filename, double[] input) {
            ...
            try {
                ...
                if (filename.endsWith(".wav") || filename.endsWith(".WAV")) {
                    ...
                    throw new RuntimeException(String.format("File format not supported: %s", filename));
                }
            } catch (Exception e) {
    ...
    private static byte[] readByte(String filename) {
        ...
        try {
            ...
            throw new RuntimeException(String.format("Could not read %s", filename));
        }
